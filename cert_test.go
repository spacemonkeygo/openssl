...
func TestCertGenerate(t *testing.T) {
	...
	if err := cert.Sign(key, "sha256"); err != nil {
		t.Fatal(err)
	}
	_, err = cert.MarshalPEM()
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestCAGenerate(t *testing.T) {
	...
	if err := ca.AddExtensions(map[int]string{
	...
	if err := ca.Sign(cakey, "sha256"); err != nil {
		t.Fatal(err)
	}
	_, err = ca.MarshalPEM()
	if err != nil {
	...
	if err := cert.AddExtensions(map[int]string{
	...
	if err := cert.Sign(cakey, "sha256"); err != nil {
		t.Fatal(err)
	}
	_, err = cert.MarshalPEM()
}
